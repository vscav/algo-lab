...
void MandelbrotThread::run()
{
    ...
    double n = qrand() % 120 + 80;
	QRgb white = qRgb(qrand() % 125 + 50, qrand() % 125 + 75, qrand() % 125 + 50);
	QRgb black = qRgb(qrand() % 20, qrand() % 30, qrand() % 20);

    double factor = 0.75 + (qrand() % 30000 - 15000) * 0.0001;
    double precision = 10000. / factor;
    double width = mandelbrotPicture->width()*precision,
           height = mandelbrotPicture->height()*precision;
    QVector2D z((qrand() % 200 - 400) / precision, (qrand() % 200 - 400) / precision);


    for (int i=0; i<mandelbrotPicture->width(); ++i)
        for (int j=0; j<mandelbrotPicture->height(); ++j)
		{
            QVector2D point(factor * (-2.75 + (3.5*precision * (double)(i)) / width),
                            factor * (-0.75 + (2.5*precision * (double)(j)) / height));
